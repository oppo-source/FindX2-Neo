...
#ifndef VENDOR_EDIT
/* Xiaoke.Zhi@PSW.MM.AudioDriver.Codec, 2020/03/12,* Add to 300ms .bug id 2838782.*/
...
struct tx_macro_priv {
	...
	int tx_mclk_users;
	#ifdef VENDOR_EDIT
	/* Le.Li@PSW.MM.AudioDriver.Codec.2571268, 2019/12/13, Add concurrency support on audio capture path */
	int va_mclk_users;
	#endif /* VENDOR_EDIT */
	int swr_clk_users;
	...
	if (enable) {
		...
		if (clk_type == VA_MCLK) {
			...
			if (tx_priv->va_mclk_users <= 0) {
				dev_err(tx_priv->dev, "%s: clock already disabled\n",
						__func__);
				tx_priv->va_mclk_users = 0;
				return 0;
			}
			tx_priv->va_mclk_users--;
			if (tx_priv->va_mclk_users == 0 && tx_priv->tx_mclk_users == 0) {
			#endif /* VENDOR_EDIT */
				regmap_update_bits(regmap,
					BOLERO_CDC_TX_CLK_RST_CTRL_FS_CNT_CONTROL,
					0x01, 0x00);
