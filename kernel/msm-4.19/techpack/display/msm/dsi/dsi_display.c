...
static void dsi_display_parse_te_data(struct dsi_display *display)
{
	...
	pr_err("display->te_source: %d", display->te_source);
}
...
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-30 add for pixelworks */
	if (display->panel->iris_clk)
		clk_prepare_enable(display->panel->iris_clk);
#endif
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
	...
	pr_err("dsi_display_dev_probe, index=%d\n", index);
...
//Zengpeng.Chen@PSW.BSP.TP, 2019/09/29, Add for notify TP display fps change
	...
	syna_tcm_refresh_switch(timing.refresh_rate);
#endif /* VENDOR_EDIT */
