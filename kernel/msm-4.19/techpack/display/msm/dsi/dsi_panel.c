...
extern int oppo_set_aod_gamma_data_status(struct dsi_panel *panel);
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-17 add for avdd */
	if (gpio_is_valid(panel->avdd_check_gpio)) {
		rc = gpio_request(panel->avdd_check_gpio, "disp_avdd_check");
		if (rc) {
			DSI_ERR("request for disp_avdd_check failed, rc=%d\n", rc);
			goto error_release_avdd_check;
		}
		gpio_direction_input(panel->avdd_check_gpio);
	}
	if (gpio_is_valid(panel->avdd_out_gpio)) {
		rc = gpio_request(panel->avdd_out_gpio, "disp_avdd_out");
		if (rc) {
			DSI_ERR("request for disp_avdd_out failed, rc=%d\n", rc);
			goto error_release_avdd_output;
		}
		gpio_direction_output(panel->avdd_out_gpio, 1);
	}

	if (gpio_is_valid(panel->iris_vdd_gpio)) {
		rc = gpio_request(panel->iris_vdd_gpio, "iris_vdd");
		if (rc) {
			DSI_WARN("request for iris vdd failed, rc=%d", rc);
			panel->iris_vdd_gpio = -1;
		}
	}

	if (gpio_is_valid(panel->abyp_gpio)) {
		rc = gpio_request(panel->abyp_gpio, "analog_bypass");
		if (rc) {
			DSI_WARN("request for iris abyp_gpio failed, rc=%d", rc);
			panel->abyp_gpio = -1;
		}
	}

	if (gpio_is_valid(panel->abyp_status_gpio)) {
		rc = gpio_request(panel->abyp_status_gpio, "analog_bypass_status");
		if (rc) {
			DSI_WARN("request for iris analog bypass status gpio failed,rc=%d", rc);
			panel->abyp_status_gpio = -1;
		}
	}

	if (gpio_is_valid(panel->iris_rst_gpio)) {
		rc = gpio_request(panel->iris_rst_gpio, "iris_reset");
		if (rc) {
			DSI_WARN("request for iris_rst_gpio failed, rc=%d", rc);
			panel->iris_rst_gpio = -1;
			goto error_release_iris_gpio;
		}
	}
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-17 add for avdd power */
error_release_iris_gpio:
	if (gpio_is_valid(panel->iris_vdd_gpio))
		gpio_free(panel->iris_vdd_gpio);
	if (gpio_is_valid(panel->abyp_gpio))
		gpio_free(panel->abyp_gpio);
	if (gpio_is_valid(panel->abyp_status_gpio))
		gpio_free(panel->abyp_status_gpio);
error_release_avdd_check:
	if (gpio_is_valid(panel->avdd_check_gpio))
		gpio_free(panel->avdd_check_gpio);
error_release_avdd_output:
	if (gpio_is_valid(panel->avdd_out_gpio))
		gpio_free(panel->avdd_out_gpio);
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-30 add for pixelworks */
	if (gpio_is_valid(panel->iris_rst_gpio)) {
		rc = gpio_direction_output(panel->iris_rst_gpio, 0);
		if (rc)
			DSI_ERR("unable to set dir for iris reset gpio rc=%d", rc);
		DSI_WARN("iris reset");
		gpio_set_value(panel->iris_rst_gpio, 0);
		usleep_range(1000, 1000);
		gpio_set_value(panel->iris_rst_gpio, 1);
...
/* Hu Jie@PSW.MM.Display.Lcd.Stability, 2019-09-27, add log at display key evevnt */
	...
	if (gpio_is_valid(panel->reset_config.disp_en_gpio))
		gpio_set_value(panel->reset_config.disp_en_gpio, 0);
#ifndef VENDOR_EDIT
//Liping-M@PSW.MM.Display.LCD.Stability,2019/10/21, add for LCD
...
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-30 add for pixelworks */
	if (gpio_is_valid(panel->iris_rst_gpio))
		gpio_set_value(panel->iris_rst_gpio, 0);
	if (gpio_is_valid(panel->iris_vdd_gpio))
		gpio_set_value(panel->iris_vdd_gpio, 0);
	if (panel->iris_clk)
		clk_disable_unprepare(panel->iris_clk);
#endif /* VENDOR_EDIT */
	if (gpio_is_valid(panel->panel_test_gpio)) {
		rc = gpio_direction_input(panel->panel_test_gpio);
		if (rc)
...
*/
	...
	if (!strcmp(panel->name,"samsung ams644va04 amoled fhd+ panel") ||
	    !strcmp(panel->name,"samsung amb655uv01 amoled fhd+ panel with DSC")) {
...
#ifdef VENDOR_EDIT
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-18 add for avdd power */
bool is_iris_support(void)
{
	if(!strcmp(get_PCB_Version(),"EVB1") ||
	   !strcmp(get_PCB_Version(),"T0") ||
	   !strcmp(get_PCB_Version(),"EVT1") ||
	   !strcmp(get_PCB_Version(),"EVT2") ||
	   !strcmp(get_PCB_Version(),"EVT3") ||
	   !strcmp(get_PCB_Version(),"DVT1") ||
	   !strcmp(get_PCB_Version(),"DVT2") ||
	   !strcmp(get_PCB_Version(),"DVT3")) {
		return true;
	}
	return false;
}

void oppo_gpio_parse(struct dsi_panel *panel)
{
	struct dsi_parser_utils *utils = &panel->utils;
	int rc = 0;
	int val = 0;

	panel->avdd_check_gpio = utils->get_named_gpio(utils->data, "oppo,avdd-check-gpio", 0);
	if (!gpio_is_valid(panel->avdd_check_gpio))
		pr_err("failed to get oppo,avdd-check-gpio\n");
	panel->avdd_out_gpio = utils->get_named_gpio(utils->data, "oppo,avdd-output-gpio", 0);
	if (!gpio_is_valid(panel->avdd_out_gpio))
		pr_err("failed to get oppo,avdd-output-gpio\n");
	rc = utils->read_u32(utils->data, "oppo,avdd-output-pulse-num", &val);
	if (rc) {
		DSI_ERR("oppo,avdd-output-pulse-num is not defined, rc=%d\n", rc);
		panel->avdd_out_plus_num = 0;
	} else {
		panel->avdd_out_plus_num = val;
	}

	rc = utils->read_u32(utils->data, "oppo,post-on-delay", &val);
	if (rc) {
		DSI_ERR("oppo,avdd-output-pulse-num is not defined, rc=%d\n", rc);
		panel->post_on_delay = 0;
	} else {
		panel->post_on_delay = val;
	}

	/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-30 add for pixel */
	if (is_iris_support()) {
		panel->abyp_gpio = utils->get_named_gpio(utils->data,
				"qcom,platform-analog-bypass-gpio", 0);
		if (!gpio_is_valid(panel->abyp_gpio))
			DSI_WARN("[%s:%d] abyp gpio not specified", __func__, __LINE__);
		panel->abyp_status_gpio = utils->get_named_gpio(utils->data,
				"qcom,platform-analog-bypass-status-gpio", 0);
		if (!gpio_is_valid(panel->abyp_status_gpio))
			DSI_WARN("[%s:%d] abyp status gpio not specified", __func__, __LINE__);
		panel->iris_rst_gpio = utils->get_named_gpio(utils->data,
				"qcom,platform-iris-reset-gpio", 0);
		if (!gpio_is_valid(panel->iris_rst_gpio))
			DSI_WARN("[%s:%d] iris reset gpio not specified", __func__, __LINE__);

		panel->iris_vdd_gpio = utils->get_named_gpio(utils->data,
				"qcom,platform-iris-vdd-gpio", 0);
		if (!gpio_is_valid(panel->iris_vdd_gpio))
			DSI_WARN("[%s:%d] iris vdd gpio not specified", __func__, __LINE__);

		/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-30 add for pw clock ctrl */
		panel->iris_clk = devm_clk_get(panel->parent, "ln_bb_clk2");
		if (IS_ERR_OR_NULL(panel->iris_clk)) {
			DSI_WARN("failed to get %s, rc=%d", "ln_bb_clk2", PTR_ERR(panel->iris_clk));
			panel->iris_clk = NULL;
		}
	} else {
		panel->abyp_gpio = -1;
		...
		panel->iris_vdd_gpio = -1;
		panel->iris_clk = NULL;
		DSI_WARN("Not support iris chips");
	}
}
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/*Mark.Yao@PSW.MM.Display.LCD.Params,2019-10-17 add for power avdd */
	oppo_gpio_parse(panel);
#endif /* VENDOR_EDIT */
...
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-30 add for pixelworks */
	if (gpio_is_valid(panel->iris_vdd_gpio)) {
		DSI_INFO("iris vdd enable");
		rc = gpio_direction_output(panel->iris_vdd_gpio, 1);
		if (rc)
			DSI_ERR("unable to set dir for iris vdd gpio rc=%d", rc);
		gpio_set_value(panel->iris_vdd_gpio, 1);
		usleep_range(2000, 2000);
	} else
		DSI_ERR("iris vdd does not valid");

	if (panel->iris_clk) {
		DSI_INFO("iris clk enable");
		clk_prepare_enable(panel->iris_clk);
		usleep_range(1000, 1000);
	} else
		DSI_ERR("iris clk does not valid");
#endif /* VENDOR_EDIT */
...
*/
	oppo_set_aod_gamma_data_status(panel);
...
#ifdef VENDOR_EDIT
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-10-17 add for avdd power */
	if (gpio_is_valid(panel->avdd_check_gpio) && gpio_is_valid(panel->avdd_out_gpio)) {
		int val = 0, i;
		int timeout;
		ktime_t old = ktime_get();
		int delay = 0;

		timeout = readx_poll_timeout_atomic(gpio_get_value, panel->avdd_check_gpio, val,
				val, 0, 20 * 1000);
		gpio_set_value(panel->avdd_out_gpio, 1);
		if (timeout)
			pr_err("wait for avdd check gpio timeout\n");
		if (panel->avdd_out_plus_num) {
			unsigned long flags;
			msleep(10);
			local_irq_save(flags);
			for (i = 0; i < panel->avdd_out_plus_num; i++) {
				gpio_set_value(panel->avdd_out_gpio, 0);
				udelay(2);
				gpio_set_value(panel->avdd_out_gpio, 1);
			}
		...
		delay = ktime_to_ms(ktime_sub(old, ktime_get()));
		if (delay < panel->post_on_delay)
			msleep(panel->post_on_delay - delay);
	}
#endif /* VENDOR_EDIT */
	if (rc)
